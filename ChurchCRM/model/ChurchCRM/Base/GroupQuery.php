 ...
 *
 * @method     \ChurchCRM\ListOptionQuery|\ChurchCRM\Person2group2roleP2g2rQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class GroupQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related \ChurchCRM\ListOption object
     *
     * @param \ChurchCRM\ListOption|ObjectCollection $listOption The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($listOption instanceof \ChurchCRM\ListOption) {
            return $this
                ->addUsingAlias(GroupTableMap::COL_GRP_TYPE, $listOption->getOptionId(), $comparison);
        } elseif ($listOption instanceof ObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(GroupTableMap::COL_GRP_TYPE, $listOption->toKeyValue('OptionId', 'OptionId'), $comparison);
        } else {
            throw new PropelException('filterByListOption() only accepts arguments of type \ChurchCRM\ListOption or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ChurchCRM\ListOptionQuery A secondary query class using the current class as primary query
     */
    public function useListOptionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinListOption($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'ListOption', '\ChurchCRM\ListOptionQuery');
    }
